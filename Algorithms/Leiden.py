...
def Leiden(dataset,direct = False):
    path = os.path.abspath(os.path.join(os.getcwd(), os.pardir)) + '/Datasets/Edge_Files/' + dataset
    Leiden_modularity_score = []
    for file in os.listdir(path):
        output_filename = os.path.abspath(os.path.join(os.getcwd(), os.pardir)) + '/CommunitiesGenerated/Leiden/' + dataset + file.split('.')[0] + '.csv'
        os.makedirs(os.path.dirname(output_filename), exist_ok=True)
        print(file)
        df = cudf.read_csv(path+file, delimiter=',', dtype=['int32','int32', 'int32'], header=0)
        ...
        Leiden_modularity_score.append(modularity_score)
    return Leiden_modularity_score

t0 = time()
modularityFacebook = Leiden('Facebook/')
averageModularityFacebook = sum(modularityFacebook)/len(modularityFacebook)
t1 = time()
modularityTwitter = Leiden('Twitter/',direct=True)
